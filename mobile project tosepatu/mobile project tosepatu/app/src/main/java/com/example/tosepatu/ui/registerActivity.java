    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        etSignupPassword.setOnTouchListener(new View.OnTouchListener() {
            public boolean onTouch(View v, MotionEvent event) {
                ...
                if (event.getAction() == MotionEvent.ACTION_UP) {
                    if (event.getRawX() >= etSignupPassword.getRight() - etSignupPassword.getCompoundDrawables()[inType].getBounds().width()) {
                        int selection = etSignupPassword.getSelectionEnd();
                        if (passwordVisible) {
                            etSignupPassword.setCompoundDrawablesWithIntrinsicBounds(0, 0, R.drawable.visibility_off, 0);
                            etSignupPassword.setTransformationMethod(PasswordTransformationMethod.getInstance());
                            passwordVisible = false;
                        } else {
                            etSignupPassword.setCompoundDrawablesWithIntrinsicBounds(0, 0, R.drawable.visibility, 0);
        ...
        etSignupConfirmPassword.setOnTouchListener(new View.OnTouchListener() {
            public boolean onTouch(View v, MotionEvent event) {
                final int inType = 2;
                if (event.getAction() == MotionEvent.ACTION_UP) {
                    if (event.getRawX() >= etSignupConfirmPassword.getRight() - etSignupConfirmPassword.getCompoundDrawables()[inType].getBounds().width()) {
                        int selection = etSignupConfirmPassword.getSelectionEnd();
                        if (passwordVisible2) {
                            etSignupConfirmPassword.setCompoundDrawablesWithIntrinsicBounds(0, 0, R.drawable.visibility_off, 0);
                            etSignupConfirmPassword.setTransformationMethod(PasswordTransformationMethod.getInstance());
                            passwordVisible2 = false;
                        } else {
                            etSignupConfirmPassword.setCompoundDrawablesWithIntrinsicBounds(0, 0, R.drawable.visibility, 0);
